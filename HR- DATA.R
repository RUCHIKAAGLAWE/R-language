...
Plan_Dataset <- read.csv("C:/Users/Dell/Downloads/R programming data/datasets/HR_Data_Tutorial 7.csv")
View(Plan_Dataset)
...
library(sqldf)
sqldf("select role, COUNT (*) as obs, avg(left) from df GROUP BY 1")
...
# ----------------------- salary --------------------------------
# To check Plan Subscription ratio in dataset
table(df$salary)/nrow(df)
# Avg Premium by state# To check Plan Subscription ratio in dataset
library(sqldf)
sqldf("select salary, COUNT (*) as obs, avg(left) from df GROUP BY 1")
#Create dummy variables for Type:
...
logit_2 <- glm(left ~ satisfaction_level+last_evaluation+#Role1+
                 Role2+
