...
matrix_c <-matrix(1:15, nrow = 5)
matrix_c

# Matrix object Properties
mx=matrix(1:30,nrow = 6)
class(mx)
dim(mx)
matrix(mx)
...
matrix_al<-cbind(matrix_a, c(1:5))
matrix_al


# Add a row to a Matrix with the rbind()
matrix_bl<-rbind(matrix_a, c(1:2))
matrix_bl
dim(matrix_al)
...
#Add names of cols and rows in matrix by using "paste" function and "sep" for sepration
