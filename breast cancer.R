...
logitmod <- glm(Class ~ Cl.thickness + #Cell.size +
                  ...
                  Bare.nuclei + Bl.cromatin + Normal.nucleoli, family = "binomial", data=trainData)

summary(logitmod)

or(y_pred_num, levels=c(0, 1))
...
#testData$y_pred <- facttestData$y_pred_num <- ifelse(pred > 0.5, 1, 0)
