 #------logistic regression-------#
  ...
  Plan_Dataset <- read.csv("C:/Users/Dell/Downloads/R programming data/datasets/Luxury_Cars (1).csv")
  View(Plan_Dataset)
  dataset<-data.frame(Plan_Dataset)
  View(dataset)
  ...
  #convert "make", "model","type", "origin", "DriveTrain", into factor
  #"model" variable is not sufficient to predict milage so remove it
  #"Cylinder" by misteck it is in the numeric so convert it into factor
  table(dataset$Model)
